...
func (s *Store) EndSession(ctx context.Context, sessionID, status string) error {
	...
	return execOne(ctx, s.db, `UPDATE sessions SET ended_at = ?, status = ? WHERE id = ? AND status = 'active' AND ended_at IS NULL AND NOT EXISTS (SELECT 1 FROM vms WHERE session_id = sessions.id AND ended_at IS NULL)`, now(), status, sessionID)
}
