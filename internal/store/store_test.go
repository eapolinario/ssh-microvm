...
func TestEnsureSchemaIsIdempotent(t *testing.T) {
	...
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 142")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 142: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 142 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 143")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 143: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 143 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 144")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 144: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 144 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 145")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 145: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 145 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 146")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 146: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 146 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 147")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 147: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 147 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 148")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 148: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 148 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 149")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 149: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 149 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 150")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 150: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 150 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 151")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 151: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 151 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 152")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 152: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 152 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 153")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 153: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 153 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 154")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 154: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 154 migration count = %d, want 1", migrationCount)
	}
	row = st.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM schema_migrations WHERE version = 155")
	if err := row.Scan(&migrationCount); err != nil {
		t.Fatalf("query schema_migrations version 155: %v", err)
	}
	if migrationCount != 1 {
		t.Fatalf("version 155 migration count = %d, want 1", migrationCount)
	}
...
func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredEightyThreeByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredEightyThreeByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAUwECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVI="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredEightyThreeByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared eighty-three-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-three-byte modulus") {
		t.Fatalf("insert key with short declared eighty-three-byte RSA modulus error = %v, want RSA declared eighty-three-byte modulus trigger error", err)
	}

	rsaKeyWithEightyThreeModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAUwECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJT"
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithEightyThreeModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with eighty-three declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredEightyThreeByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared eighty-three-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-three-byte modulus") {
		t.Fatalf("update key to short declared eighty-three-byte RSA modulus error = %v, want RSA declared eighty-three-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithEightyThreeModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithEightyThreeModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredEightyFourByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredEightyFourByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAVAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJT"
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredEightyFourByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared eighty-four-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-four-byte modulus") {
		t.Fatalf("insert key with short declared eighty-four-byte RSA modulus error = %v, want RSA declared eighty-four-byte modulus trigger error", err)
	}

	rsaKeyWithEightyFourModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAVAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVA=="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithEightyFourModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with eighty-four declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredEightyFourByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared eighty-four-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-four-byte modulus") {
		t.Fatalf("update key to short declared eighty-four-byte RSA modulus error = %v, want RSA declared eighty-four-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithEightyFourModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithEightyFourModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredEightyFiveByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredEightyFiveByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAVQECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVA=="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredEightyFiveByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared eighty-five-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-five-byte modulus") {
		t.Fatalf("insert key with short declared eighty-five-byte RSA modulus error = %v, want RSA declared eighty-five-byte modulus trigger error", err)
	}

	rsaKeyWithEightyFiveModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAVQECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFU="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithEightyFiveModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with eighty-five declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredEightyFiveByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared eighty-five-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-five-byte modulus") {
		t.Fatalf("update key to short declared eighty-five-byte RSA modulus error = %v, want RSA declared eighty-five-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithEightyFiveModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithEightyFiveModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredEightySixByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredEightySixByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAVgECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFU="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredEightySixByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared eighty-six-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-six-byte modulus") {
		t.Fatalf("insert key with short declared eighty-six-byte RSA modulus error = %v, want RSA declared eighty-six-byte modulus trigger error", err)
	}

	rsaKeyWithEightySixModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAVgECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVW"
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithEightySixModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with eighty-six declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredEightySixByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared eighty-six-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-six-byte modulus") {
		t.Fatalf("update key to short declared eighty-six-byte RSA modulus error = %v, want RSA declared eighty-six-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithEightySixModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithEightySixModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredEightySevenByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredEightySevenByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAVwECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVW"
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredEightySevenByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared eighty-seven-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-seven-byte modulus") {
		t.Fatalf("insert key with short declared eighty-seven-byte RSA modulus error = %v, want RSA declared eighty-seven-byte modulus trigger error", err)
	}

	rsaKeyWithEightySevenModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAVwECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWVw=="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithEightySevenModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with eighty-seven declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredEightySevenByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared eighty-seven-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-seven-byte modulus") {
		t.Fatalf("update key to short declared eighty-seven-byte RSA modulus error = %v, want RSA declared eighty-seven-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithEightySevenModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithEightySevenModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredEightyEightByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredEightyEightByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAWAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWVw=="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredEightyEightByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared eighty-eight-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-eight-byte modulus") {
		t.Fatalf("insert key with short declared eighty-eight-byte RSA modulus error = %v, want RSA declared eighty-eight-byte modulus trigger error", err)
	}

	rsaKeyWithEightyEightModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAWAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1g="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithEightyEightModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with eighty-eight declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredEightyEightByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared eighty-eight-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-eight-byte modulus") {
		t.Fatalf("update key to short declared eighty-eight-byte RSA modulus error = %v, want RSA declared eighty-eight-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithEightyEightModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithEightyEightModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredEightyNineByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredEightyNineByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAWQECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1g="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredEightyNineByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared eighty-nine-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-nine-byte modulus") {
		t.Fatalf("insert key with short declared eighty-nine-byte RSA modulus error = %v, want RSA declared eighty-nine-byte modulus trigger error", err)
	}

	rsaKeyWithEightyNineModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAWQECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZ"
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithEightyNineModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with eighty-nine declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredEightyNineByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared eighty-nine-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared eighty-nine-byte modulus") {
		t.Fatalf("update key to short declared eighty-nine-byte RSA modulus error = %v, want RSA declared eighty-nine-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithEightyNineModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithEightyNineModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredNinetyByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredNinetyByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAWgECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZ"
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredNinetyByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared ninety-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-byte modulus") {
		t.Fatalf("insert key with short declared ninety-byte RSA modulus error = %v, want RSA declared ninety-byte modulus trigger error", err)
	}

	rsaKeyWithNinetyModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAWgECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWg=="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithNinetyModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with ninety declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredNinetyByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared ninety-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-byte modulus") {
		t.Fatalf("update key to short declared ninety-byte RSA modulus error = %v, want RSA declared ninety-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithNinetyModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithNinetyModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredNinetyOneByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredNinetyOneByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAWwECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWg=="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredNinetyOneByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared ninety-one-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-one-byte modulus") {
		t.Fatalf("insert key with short declared ninety-one-byte RSA modulus error = %v, want RSA declared ninety-one-byte modulus trigger error", err)
	}

	rsaKeyWithNinetyOneModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAWwECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWls="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithNinetyOneModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with ninety-one declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredNinetyOneByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared ninety-one-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-one-byte modulus") {
		t.Fatalf("update key to short declared ninety-one-byte RSA modulus error = %v, want RSA declared ninety-one-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithNinetyOneModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithNinetyOneModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredNinetyTwoByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredNinetyTwoByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAXAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWls="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredNinetyTwoByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared ninety-two-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-two-byte modulus") {
		t.Fatalf("insert key with short declared ninety-two-byte RSA modulus error = %v, want RSA declared ninety-two-byte modulus trigger error", err)
	}

	rsaKeyWithNinetyTwoModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAXAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltc"
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithNinetyTwoModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with ninety-two declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredNinetyTwoByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared ninety-two-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-two-byte modulus") {
		t.Fatalf("update key to short declared ninety-two-byte RSA modulus error = %v, want RSA declared ninety-two-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithNinetyTwoModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithNinetyTwoModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredNinetyThreeByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredNinetyThreeByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAXQECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltc"
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredNinetyThreeByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared ninety-three-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-three-byte modulus") {
		t.Fatalf("insert key with short declared ninety-three-byte RSA modulus error = %v, want RSA declared ninety-three-byte modulus trigger error", err)
	}

	rsaKeyWithNinetyThreeModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAXQECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltcXQ=="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithNinetyThreeModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with ninety-three declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredNinetyThreeByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared ninety-three-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-three-byte modulus") {
		t.Fatalf("update key to short declared ninety-three-byte RSA modulus error = %v, want RSA declared ninety-three-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithNinetyThreeModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithNinetyThreeModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredNinetyFourByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredNinetyFourByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAXgECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltcXQ=="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredNinetyFourByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared ninety-four-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-four-byte modulus") {
		t.Fatalf("insert key with short declared ninety-four-byte RSA modulus error = %v, want RSA declared ninety-four-byte modulus trigger error", err)
	}

	rsaKeyWithNinetyFourModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAXgECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltcXV4="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithNinetyFourModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with ninety-four declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredNinetyFourByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared ninety-four-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-four-byte modulus") {
		t.Fatalf("update key to short declared ninety-four-byte RSA modulus error = %v, want RSA declared ninety-four-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithNinetyFourModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithNinetyFourModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredNinetyFiveByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredNinetyFiveByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAXwECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltcXV4="
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredNinetyFiveByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared ninety-five-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-five-byte modulus") {
		t.Fatalf("insert key with short declared ninety-five-byte RSA modulus error = %v, want RSA declared ninety-five-byte modulus trigger error", err)
	}

	rsaKeyWithNinetyFiveModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAXwECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltcXV5f"
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithNinetyFiveModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with ninety-five declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredNinetyFiveByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared ninety-five-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-five-byte modulus") {
		t.Fatalf("update key to short declared ninety-five-byte RSA modulus error = %v, want RSA declared ninety-five-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithNinetyFiveModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithNinetyFiveModulusBytes)
	}
}

func TestEnsureSchemaEnforcesKeyPublicKeyRSABlobDeclaredNinetySixByteModulus(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	...
	rsaKeyWithShortDeclaredNinetySixByteModulus := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAYAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltcXV5f"
	_, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testSHA256Fingerprint('A'), "user-1", rsaKeyWithShortDeclaredNinetySixByteModulus, now(), now())
	if err == nil {
		t.Fatalf("inserted key with short declared ninety-six-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-six-byte modulus") {
		t.Fatalf("insert key with short declared ninety-six-byte RSA modulus error = %v, want RSA declared ninety-six-byte modulus trigger error", err)
	}

	rsaKeyWithNinetySixModulusBytes := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAYAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0+P0BBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWltcXV5fYA=="
	if _, err := st.db.ExecContext(ctx, `INSERT INTO keys(fingerprint, user_id, public_key, added_at, last_seen_at)
VALUES(?, ?, ?, ?, ?)`, testKeyFingerprint, "user-1", rsaKeyWithNinetySixModulusBytes, now(), now()); err != nil {
		t.Fatalf("insert valid RSA key with ninety-six declared modulus bytes: %v", err)
	}
	_, err = st.db.ExecContext(ctx, "UPDATE keys SET public_key = ? WHERE fingerprint = ?", rsaKeyWithShortDeclaredNinetySixByteModulus, testKeyFingerprint)
	if err == nil {
		t.Fatalf("updated key to short declared ninety-six-byte RSA modulus, want trigger error")
	}
	if !strings.Contains(err.Error(), "ssh-rsa public key blob must include declared ninety-six-byte modulus") {
		t.Fatalf("update key to short declared ninety-six-byte RSA modulus error = %v, want RSA declared ninety-six-byte modulus trigger error", err)
	}
	...
	if gotPublicKey != rsaKeyWithNinetySixModulusBytes {
		t.Fatalf("key public_key = %q, want %q", gotPublicKey, rsaKeyWithNinetySixModulusBytes)
	}
}
